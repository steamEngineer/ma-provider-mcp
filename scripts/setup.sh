...
# Dev environment setup for the fastmcp_server provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/fastmcp_server"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: fastmcp_server → $TARGET"
  ...
  echo "Provider fastmcp_server linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
