...
# Dev environment setup for the mcp_server provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/mcp_server"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: mcp_server → $TARGET"
  ...
  echo "Provider mcp_server linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
